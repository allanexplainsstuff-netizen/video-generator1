// Updated Gemini Vision Service - Now calls server-side endpoint
...
class GeminiVisionService {
    ...
    // Call server-side Gemini endpoint instead of direct API
    async analyzeImageWithText(imageData, userPrompt) {
        console.log('Gemini Vision Service: Calling server-side endpoint', { 
        ...
        try {
            ...
            if (!data.success) {
                // Server returned fallback indication
            ...
            console.log('Gemini Vision Service: Server-side analysis completed', data.enhancedPrompt);
            return data.enhancedPrompt;

        } catch (error) {
            console.error('Gemini Vision Service: Server call failed', error);
            throw error; // Let caller handle fallback
        }
    }

    // Fallback method (kept for emergency use)
    generateBasicImageDescription(userPrompt) {
        const sceneTypes = [
            'cinematic wide shot with professional lighting',
            'detailed close-up with dramatic atmosphere',
            'dynamic tracking shot with smooth movement',
            'atmospheric scene with moody lighting'
        ];
        
        const randomScene = sceneTypes[Math.floor(Math.random() * sceneTypes.length)];
        return `${userPrompt}, ${randomScene}, high quality video production`;
    }
}
